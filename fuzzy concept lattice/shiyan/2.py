...
def main():
    ...
    choices = [0, 0.5, 1]
    a = [[random.choice(choices) for _ in range(m)] for _ in range(n_1)]
    b = [[random.choice(choices) for _ in range(m)] for _ in range(n_2)]
    ...
    all_cpt_1 = get_all_cpt(table3, a, x_list_n_1)
    all_cpt_2 = get_all_cpt(table3, b, x_list_n_2)
    ...
    #  总结果 5
    x_list_nm = get_all_sort_x(n_1 + n_2, t3)
    ...
    combine_result = combine_concept(all_cpt_1, all_cpt_2)
