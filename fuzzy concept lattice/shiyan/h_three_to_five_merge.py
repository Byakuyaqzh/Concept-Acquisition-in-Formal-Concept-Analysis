...
def main():
    ...
    choices = [0, 0.5, 1]
    a = [[random.choice(choices) for _ in range(m_1)] for _ in range(n)]
    b = [[random.choice(choices) for _ in range(m_2)] for _ in range(n)]
    c = []
    for i in range(n):
        c.append(a[i] + b[i])
    ...
    #  x 所有可能的排列   第一个3  第二个5
    x_list_1 = get_all_sort_x(n, t3)
    #  通过 表、模糊形式背景、x所有可能的排列，计算所有概念
    all_cpt_1 = get_all_cpt(table3, a, x_list_1)

    front_cpt_end_time = time.perf_counter()

    front_cpt_start_time_ = time.perf_counter()

    x_list_2 = get_all_sort_x(n, t5)
    all_cpt_2 = get_all_cpt(table5, b, x_list_2)
    ...
    #  总结果 5
    x_list_3 = get_all_sort_x(n, t5)
    all_cpt_3 = get_all_cpt(table5, c, x_list_3)
    ...
    all_cpt_1_35 = three_to_five_concept(all_cpt_1)
    ...
    for i in range(num):
        combine_result.append(concept(all_cpt_1_35[i].X, all_cpt_1_35[i].Y + all_cpt_2[i].Y))
